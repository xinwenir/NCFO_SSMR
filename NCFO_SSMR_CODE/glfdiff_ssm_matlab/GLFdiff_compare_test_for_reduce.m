...
[dx_wpwl] = GLF_wPWL(u,t,alpha,h,1024);
...
legend('Theoretical','L=1024','L_Linear','L_Quadratic','PWL','PWQEF','W-PWLEF');
...
e_dx_wpwl = abs(dx_o - dx_wpwl);
    
e_l_1024_max = vpa(max(e_dx_1024_l),5);
%e_L_Linear_max = max(e_dx_linear);
%e_L_Quadratic_max = max(e_dx_quadratic);
e_PWL_max = vpa(max(e_dx_pwl),5);
e_PWQEF_max = vpa(max(e_dx_pwqef),5);
e_dx_wPWL_max = vpa(max(e_dx_wpwl),5);
fprintf(' &%.1f &%.4f &%.4f &%.4f &%.4f\n',alpha,e_l_1024_max,e_PWL_max,e_PWQEF_max,e_dx_wPWL_max);
subplot(1,2,2);
