function [x,y] = fossm(u,t,A,B,C,D,x0,alpha,h,r) %u = xxxx*t.t = 0 : 1 : 1000
    %arguments, u(:,1) single, t(1,1) int32, A(:,:),B(:,:),C(:,:),D(:,:) single, x0(:,1), alpha(:,1) single, h(1,1) single, r(1,:) int32, end
    FO_N = length(r);
    w = ones(FO_N,1);
    DIM = sum(r);
    x = zeros(DIM,1,'double');
    x_q = zeros(DIM,1,'double');
    dx = zeros(DIM,1,'double');
    times = 0;
    for i = 1 : FO_N
        for j = 2 : t + 1 
        ...
        if i == 1
            ...
            for foi = 1 : FO_N
                for j = 1 : r(foi)
                    x_q = rot90(x(k,1 : i-1));
                    %x_q
                    dx(k) = glfdiff_sim(x_q,i,w(foi,2:t+1));                  
                    k = k + 1;              
                end
            end           
