function [x,y] = fossm_PWL(u,t,A,B,C,D,x0,alpha,h,r,L) %u = xxxx*t.t = 0 : 1 : 1000
...
%         end
        %[w(i,:),w_linear(i,:)] = w_linear_gen(L,h,Thn,alpha(i)); 
        [w, w_qf, qsp, w_lf(i,:),lsp] = w_gen(alpha(i),h,Thn,L,"linear");
    end  
    for i = 1 : FO_N 
        w_lf(i,t+1) = w_lf(i,t);
    end
        ...
        if i == 1
            ...
            for foi = 1 : FO_N
                for j = 1 : r(foi)
                    x_q = rot90(x(k,1 : i-1));
                    %x_q
                    dx(k) = glfdiff_sim(x_q,i,w_lf(foi,2:t+1)); 
                    %dx(k) = glfdiff_sim_l(x_q,i,w_lf(foi,2:L+1),L); 
                    k = k + 1;              
                end
            end           
