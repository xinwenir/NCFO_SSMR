function [x,y] = fossm_l_linear(u,t,A,B,C,D,x0,alpha,h,r,L) %u = xxxx*t.t = 0 : 1 : 1000
    ...
    for i = 1 : FO_N
        [w,w_linear(i,:), s_LinearFit_p, w_quadratic, s_quadraticFit_p] = w_linear_quadratic_gen(L,h,Thn,alpha(i));     
    end  
    for i = 1 : FO_N 
        w_linear(i,t+1) = w_linear(i,t);
    end
    h_m = ones(FO_N,1);
    k = 1;
    for i = 1 : FO_N
        ...
        if i == 1
            ...
            for foi = 1 : FO_N
                for j = 1 : r(foi)
                    x_q = rot90(x(k,1 : i-1));
                    %x_q
                    dx(k) = glfdiff_sim(x_q,i,w_linear(foi,2:t+1));                  
                    k = k + 1;              
                end
            end           
