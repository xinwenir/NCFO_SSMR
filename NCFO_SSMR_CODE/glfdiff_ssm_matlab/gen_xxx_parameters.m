function [c_sx] = gen_xxx_parameters(t,alpha,h,r,L,type)
    FO_N = length(r);
    Thn = t * h;
    %w = ones(FO_N,1);
    DIM = sum(r);
    ...
    elseif type == "linear"
        for i = 1 : FO_N
            [w, w_qf, qsp, w_lf, lsp(i,:,:)] = w_gen(alpha(i),h,Thn,L,"linear");
        end       
        if     L == 256
            L_Contain = [L0, L1, L2, L3];% 256 ----> end
        elseif L == 512
        ...
        for i = 1 : FO_N
            for j = 1 : r(i)
                for L_index = 1 : (length(L_Contain))
                    n = L_Contain(L_index);
                    Lm = L_Contain(L_index);
                    c_s1b = Lm * lsp(i,L_index,1) * h; %L*p1*h ==>--    
                    c_s3a = lsp(i,L_index,1) * n * h + lsp(i,L_index,2); %p1*(jh) + p3
                    c_s3b = lsp(i,L_index,1) * h; %p1*h
    
                    c_sx_q = [c_s1b; c_s3a; c_s3b];
