function [w, w_qf, qsp, w_lf,lsp] = w_gen(gam,h,Thn,L,type) % The minimum value of L is 256
t = h : h : Thn;
w = 1.0;
% Lend = 4096;
if exist("Lend","var")
...
if type == "quadratic"
    ...
    %% 二次项拟合
    [qfres0, ] = createFit_Quadratic(t(L0 : L1 - 1), w(L0 : L1 - 1));    % 32  ----> 63
    [qfres1, ] = createFit_Quadratic(t(L1 : L2 - 1), w(L1 : L2 - 1));    % 64  ----> 127
    [qfres2, ] = createFit_Quadratic(t(L2 : L3 - 1), w(L2 : L3 - 1));    % 128 ----> 255
    if     L == 256
        [qfres3, ] = createFit_Quadratic(t(L3 : end), w(L3 : end));      % 256 ----> end
    elseif L == 512
...
elseif type == "linear"
    ...
    [lfres1, ] = createFit_Linear(t(L0 + 1 : L1 - 1), w(L0 + 1 : L1 - 1));% 32   ----> 64
    [lfres2, ] = createFit_Linear(t(L1 : L2 - 1), w(L1 : L2 - 1));% 64   ----> 128
    [lfres3, ] = createFit_Linear(t(L2 : L3 - 1), w(L2 : L3 - 1));% 128   ----> 256
    if     L == 256
        [lfres4, ] = createFit_Linear(t(L3 : end), w(L3 : end));% 256 ----> end
    elseif L == 512
        [lfres4, ] = createFit_Linear(t(L3 : L4 - 1), w(L3 : L4 - 1));% 256   ----> 512
        [lfres5, ] = createFit_Linear(t(L4 : end), w(L4 : end));% 512   ----> end
    elseif L == 1024
        [lfres4, ] = createFit_Linear(t(L3 : L4 - 1), w(L3 : L4 - 1));% 256   ----> 512
        [lfres5, ] = createFit_Linear(t(L4 : L5 - 1), w(L4 : L5 - 1));% 512   ----> 1024
        [lfres6, ] = createFit_Linear(t(L5 : end), w(L5 : end));% 1024   ----> end
    elseif L == 2048
        [lfres4, ] = createFit_Linear(t(L3 : L4 - 1), w(L3 : L4 - 1));% 256   ----> 512
        [lfres5, ] = createFit_Linear(t(L4 : L5 - 1), w(L4 : L5 - 1));% 512   ----> 1024
        [lfres6, ] = createFit_Linear(t(L5 : L6 - 1), w(L5 : L6 - 1));% 1024   ----> 2048
        if exist("Lend","var")
            [lfres7, ] = createFit_Linear(t(L6 : Lend), w(L6 : Lend));% 2048   ----> end
        else
            [lfres7, ] = createFit_Linear(t(L6 : end), w(L6 : end));% 2048   ----> end
        end
    elseif L == 4096
        [lfres4, ] = createFit_Linear(t(L3 : L4 - 1), w(L3 : L4 - 1));% 256   ----> 512
        [lfres5, ] = createFit_Linear(t(L4 : L5 - 1), w(L4 : L5 - 1));% 512   ----> 1024
        [lfres6, ] = createFit_Linear(t(L5 : L6 - 1), w(L5 : L6 - 1));% 1024   ----> 2048
        [lfres7, ] = createFit_Linear(t(L6 : L7 - 1), w(L6 : L7 - 1));% 2048   ----> 4096
        [lfres8, ] = createFit_Linear(t(L7 : end), w(L7 : end));% 4096   ----> end
    end
    ...
    if     L == 256
        lsp = [ls1p; ls2p; ls3p; ls4p];   % 256 ----> end
    elseif L == 512
        lsp = [ls1p; ls2p; ls3p; ls4p; ls5p];   % 512 ----> end
    elseif L == 1024
        lsp = [ls1p; ls2p; ls3p; ls4p; ls5p; ls6p];   % 1024   ----> end
    elseif L == 2048
        lsp = [ls1p; ls2p; ls3p; ls4p; ls5p; ls6p; ls7p];   % 2048   ----> end
    elseif L == 4096
        lsp = [ls1p; ls2p; ls3p; ls4p; ls5p; ls6p; ls7p; ls8p];   % 4096 ----> end
    end
...
%     end
    ...
    if     L == 256
        ...
        for i = 2 : length(t)
            ...
            elseif i > L0 && i < L1
                w_lf(L0 + 1 : L1 - 1) = lfres1.p1 .* t(L0 + 1 : L1 - 1) + lfres1.p2;
            elseif i >= L1 && i < L2
                w_lf(L1 : L2 - 1) = lfres2.p1 .* t(L1 : L2 - 1) + lfres2.p2;
            elseif i >= L2 && i < L3
                w_lf(L2 : L3 - 1) = lfres3.p1 .* t(L2 : L3 - 1) + lfres3.p2;
            elseif i >= L3
                w_lf(L3 : end) = lfres4.p1 .* t(L3 : end) + lfres4.p2;
            end
        end
    elseif L == 512
        % 512 ----> end
        for i = 2 : length(t)
            ...
            elseif i >= L2 && i < L3
                w_lf(L2 : L3 - 1) = lfres3.p1 .* t(L2 : L3 - 1) + lfres3.p2;
            elseif i >= L3 && i < L4
                w_lf(L3 : L4 - 1) = lfres4.p1 .* t(L3 : L4 - 1) + lfres4.p2;
            elseif i >= L4
                w_lf(L4 : end) = lfres5.p1 .* t(L4 : end) + lfres5.p2;
            end
        end
    elseif L == 1024
        % 1024   ----> end
        for i = 2 : length(t)
            ...
            elseif i >= L3 && i < L4
                w_lf(L3 : L4 - 1) = lfres4.p1 .* t(L3 : L4 - 1) + lfres4.p2;
            elseif i >= L4 && i < L5
                w_lf(L4 : L5 - 1) = lfres5.p1 .* t(L4 : L5 - 1) + lfres5.p2;
            elseif i >= L5
                w_lf(L5 : end) = lfres6.p1 .* t(L5 : end) + lfres6.p2;
            end
        end
    elseif L == 2048
        % 2048   ----> end
        for i = 2 : length(t)
            ...
            elseif i >= L4 && i < L5
                w_lf(L4 : L5 - 1) = lfres5.p1 .* t(L4 : L5 - 1) + lfres5.p2;
            elseif i >= L5 && i < L6
                w_lf(L5 : L6 - 1) = lfres6.p1 .* t(L5 : L6 - 1) + lfres6.p2;
            elseif i >= L6
                if exist("Lend","var")
                    w_lf(L6 : Lend) = lfres7.p1 .* t(L6 : Lend) + lfres7.p2;
                else
                    w_lf(L6 : end) = lfres7.p1 .* t(L6 : end) + lfres7.p2;
                end
            end
        end
    elseif L == 4096
        ...
        for i = 2 : length(t)
            ...
            elseif i >= L5 && i < L6
                w_lf(L5 : L6 - 1) = lfres6.p1 .* t(L5 : L6 - 1) + lfres6.p2;
            elseif i >= L6 && i< L7
                w_lf(L6 : L7 - 1) = lfres7.p1 .* t(L6 : L7 - 1) + lfres7.p2;
            elseif i >= L7
                w_lf(L7 : end) = lfres8.p1 .* t(L7 : end) + lfres8.p2;
            end
        end
    end
%     w_lf (1:L0) = w(1:L0);
...
%
% subplot(2,3,4);
% plot(n(L4 + 1 : L5),w(L4 + 1 : L5),'r','LineWidth',3);
% hold on
% plot(n(L4 + 1 : L5),w_qf(L4 + 1 : L5),'b','LineWidth',3);
% hold on
% plot(n(L4 + 1 : L5),w_lf(L4 + 1 : L5),'g','LineWidth',3);
% hold on
% title('(d) L4 -- L5','fontname','Times New Roman','Color','b','FontSize',16);
% xlabel('n','fontname','Times New Roman','FontSize',16);
% ylabel('w','fontname','Times New Roman','FontSize',14);
% set(gca,'FontName','Times New Roman','FontSize',16);
% legend('w','wqf','wlf');
%
% subplot(2,3,5);
% plot(n(L5 + 1 : L6),w(L5 + 1 : L6),'r','LineWidth',3);
% hold on
% plot(n(L5 + 1 : L6),w_qf(L5 + 1 : L6),'b','LineWidth',3);
% hold on
% plot(n(L5 + 1 : L6),w_lf(L5 + 1 : L6),'g','LineWidth',3);
% hold on
% title('(e) L5 -- L6','fontname','Times New Roman','Color','b','FontSize',16);
% xlabel('n','fontname','Times New Roman','FontSize',16);
% ylabel('w','fontname','Times New Roman','FontSize',14);
% set(gca,'FontName','Times New Roman','FontSize',16);
% legend('w','wqf','wlf');
%
% subplot(2,3,6);
% plot(n(L6 + 1 : L7),w(L6 + 1 : L7),'r','LineWidth',3);
% hold on
% plot(n(L6 + 1 : L7),w_qf(L6 + 1 : L7),'b','LineWidth',3);
% hold on
% plot(n(L6 + 1 : L7),w_lf(L6 + 1 : L7),'g','LineWidth',3);
% hold on
% title('(f) L6 -- L7','fontname','Times New Roman','Color','b','FontSize',16);
% xlabel('n','fontname','Times New Roman','FontSize',16);
% ylabel('w','fontname','Times New Roman','FontSize',14);
% set(gca,'FontName','Times New Roman','FontSize',16);
% legend('w','wqf','wlf');
...
%
% subplot(2,3,4);
% plot(n(L4 + 1 : L5),e_w_qf(L4 + 1 : L5),'r','LineWidth',3);
% hold on
% plot(n(L4 + 1 : L5),e_w_lf(L4 + 1 : L5),'b','LineWidth',3);
% hold on
% title('(d) L4 -- L5','fontname','Times New Roman','Color','b','FontSize',16);
% xlabel('n','fontname','Times New Roman','FontSize',16);
% ylabel('w','fontname','Times New Roman','FontSize',16);
...
%
% subplot(2,3,5);
% plot(n(L5 + 1 : L6),e_w_qf(L5 + 1 : L6),'r','LineWidth',3);
% hold on
% plot(n(L5 + 1 : L6),e_w_lf(L5 + 1 : L6),'b','LineWidth',3);
% hold on
% title('(e) L5 -- L6','fontname','Times New Roman','Color','b','FontSize',16);
% xlabel('n','fontname','Times New Roman','FontSize',16);
% ylabel('w','fontname','Times New Roman','FontSize',16);
...
%
% subplot(2,3,6);
% plot(n(L6 + 1 : L7),e_w_qf(L6 + 1 : L7),'r','LineWidth',3);
% hold on
% plot(n(L6 + 1 : L7),e_w_lf(L6 + 1 : L7),'b','LineWidth',3);
% hold on
% title('(f) L6 -- L7','fontname','Times New Roman','Color','b','FontSize',16);
% xlabel('n','fontname','Times New Roman','FontSize',16);
% ylabel('w','fontname','Times New Roman','FontSize',16);
